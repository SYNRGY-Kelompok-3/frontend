...
const Seat = ({ rows, renderSeat }: SeatProps) => {
  return (
    ...
    <div className="flex w-[324px] justify-between font-semibold text-black pt-[312px] pl-2 relative">
      <div>
        {rows.map((row) => (
          <div key={`column1-${row}`} className="flex items-center justify-center mb-4 ">
            {["A", "B", "C"].map((seat) => renderSeat({ seat, row }))}
          </div>
        ))}
        {/* Seats Footer */}
        <div className="flex">
      ...
      <div>
        {rows.map((row) => (
          <div key={`column2-${row}`} className="flex items-center mb-4">
            {["D", "E", "F"].map((seat) => renderSeat({ seat, row }))}
          </div>
        ))}
        <div className="flex">
